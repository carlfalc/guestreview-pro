...
export function DuplicateWizard({
  ...
  useEffect(() => {
    ...
    supabase
      .from("businesses")
      .select("id, name, brand_primary, logo_url, google_review_url")
      .order("name")
      .then(({ data, error }) => {
        setBusy(null);
  ...
  useEffect(() => {
    ...
    supabase
      .from("qr_codes")
      .select("id, short_code, label, destination_type, destination_url, business_id")
  ...
  async function create() {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        status: "draft",
        selected_formats: fields.formats
          ? (src.selected_formats as unknown as never)
          : ([] as unknown as never),
      };
      ...
      const cloned = JSON.parse(JSON.stringify(src.format_customizations ?? {})) as Record<
        string,
      ...
      <DialogContent className="max-w-lg">
        ...
        {mode === "another-business" && step === 1 && (
          <div className="space-y-2">
            ...
            {busy === "businesses" ? (
              <BusyRow>Loading businesses…</BusyRow>
            ) : (
              <RadioGroup
                value={businessId}
                onValueChange={(v) => {
              ...
              >
                {businesses.map((b) => (
                  <label
                    key={b.id}
                    className="flex cursor-pointer items-center gap-2 rounded-lg border border-border/60 px-2 py-1.5 text-xs hover:bg-accent"
                  >
        ...
        {((mode === "another-qr" && step === 1) || (mode === "another-business" && step === 2)) && (
          <div className="space-y-2">
            ...
            {busy === "qrs" ? (
              <BusyRow>Loading QR codes…</BusyRow>
            ) : qrs.length === 0 ? (
              <p className="rounded-lg bg-accent/30 p-3 text-xs text-muted-foreground">
                No QR codes on this business.
              </p>
            ) : (
              <RadioGroup
                value={qrId ?? ""}
                onValueChange={setQrId}
                className="max-h-[40vh] space-y-1 overflow-y-auto"
              >
                {qrs.map((q) => (
                  <label
        ...
        {((mode === "another-qr" && step === 4) || (mode === "another-business" && step === 6)) && (
          <ConfirmStep
            ...
            qrLabel={
              qrs.find((q) => q.id === qrId)?.label ??
              qrs.find((q) => q.id === qrId)?.short_code ??
              "—"
            }
            fields={fields}
            brandMode={brandMode}
          ...
          <div className="flex gap-2">
            ...
            {step < totalSteps ? (
              <Button
                ...
                disabled={
                  busy != null ||
                  (step === 1 && mode === "another-qr" && !qrId) ||
                  (step === 2 && mode === "another-business" && !qrId)
                }
                className="rounded-full"
              >
                Next
...
function BusyRow({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2 rounded-lg bg-accent/30 p-3 text-xs text-muted-foreground">
...
function FieldsPicker({
  ...
  onChange: (f: DuplicateFields) => void;
}) {
  const items: { k: keyof DuplicateFields; label: string; hint: string }[] = [
    {
      k: "content",
      label: "Written content",
      hint: "Headline, support text, CTA, footer, visibility toggles.",
    },
    {
      k: "design",
      label: "Global design",
      hint: "Layout template + global colours, fonts, borders.",
    },
    { k: "formats", label: "Formats", hint: "The list of selected formats." },
    {
      k: "customizations",
      label: "Per-format customisations",
      hint: "Overrides you've set on individual formats (non-folded fields).",
    },
    {
      k: "folded",
      label: "Folded front / back content",
      hint: "Per-panel content for table tents. Copied independently of per-format customisations.",
    },
    {
      k: "backgroundImages",
      label: "Background images",
      hint: "Uploaded images (global, per-format, and folded front/back).",
    },
  ];
  return (
    <div className="space-y-1.5">
      ...
      {items.map((it) => (
        <label
          key={it.k}
          className="flex cursor-pointer items-start gap-2 rounded-lg border border-border/60 p-2 text-xs hover:bg-accent"
        >
          <Checkbox
            className="mt-0.5"
            checked={fields[it.k]}
            onCheckedChange={(v) => onChange({ ...fields, [it.k]: v === true })}
          />
          <div>
      ...
      <p className="rounded-lg bg-accent/40 p-2 text-[10px] text-muted-foreground">
        <span className="font-semibold">Per-format customisations</span> and{" "}
        <span className="font-semibold">Folded content</span> are independent. Enable folded alone
        to copy only table-tent panel content; enable per-format alone to copy non-folded overrides.
      </p>
    </div>
  );
}
...
function BrandStep({
  ...
  value: BrandBehaviour;
  onChange: (v: BrandBehaviour) => void;
}) {
  return (
    <div className="space-y-2">
      ...
      <RadioGroup
        value={value}
        onValueChange={(v) => onChange(v as BrandBehaviour)}
        className="space-y-1"
      >
        <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-border/60 p-2 text-xs hover:bg-accent">
          ...
          <div>
            <p className="font-medium">Replace with new business branding (recommended)</p>
            <p className="text-[10px] text-muted-foreground">
              Use the destination business's name, logo and brand colour.
            </p>
          </div>
        </label>
        <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-border/60 p-2 text-xs hover:bg-accent">
          ...
          <div>
            <p className="font-medium">Preserve current colours</p>
            <p className="text-[10px] text-muted-foreground">
              Keep the source pack's colours and design; only the QR destination changes.
            </p>
          </div>
        </label>
        <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-border/60 p-2 text-xs hover:bg-accent">
          ...
          <div>
            <p className="font-medium">Use new logo but preserve design colours</p>
            <p className="text-[10px] text-muted-foreground">
              Adopt the destination logo and business name but keep custom colours.
            </p>
          </div>
        </label>
      </RadioGroup>
  ...
  return (
    <div className="space-y-2 text-xs">
      <div className="rounded-xl bg-accent/30 p-3">
        <p>
          <span className="font-semibold">Destination:</span> {businessName} · {qrLabel}
        </p>
        <p>
          <span className="font-semibold">Name:</span> {projectName}
        </p>
        <p>
          <span className="font-semibold">Copying:</span>{" "}
          {rows.length ? rows.join(", ") : "nothing (empty pack)"}
        </p>
        {mode === "another-business" && (
          <p>
            <span className="font-semibold">Brand behaviour:</span>{" "}
            {brandMode === "replace"
              ? "Replace with new business branding"
              : brandMode === "preserve"
                ? "Preserve current colours"
                : "New logo, preserved colours"}
          </p>
        )}
      </div>
      <p className="text-[10px] text-muted-foreground">
        Scan events, analytics, review-click history, export status and preview thumbnails are not
        copied. The new pack starts as Draft and will render its own preview on first open.
      </p>
    </div>
  );
}
