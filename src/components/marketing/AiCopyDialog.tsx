...
export function AiCopyDialog({
  ...
  onApply: (
    patch: ApplyPatch,
    source: { alternativeIndex: number; generationId: string | null },
  ) => void;
  onUndo: () => void;
  canUndo: boolean;
}) {
  ...
  const [businessType, setBusinessType] = useState<BusinessType>(
    ctx.businessType ?? "General business",
  );
  ...
  async function run() {
    ...
    try {
      const input: GenerateInput = {
        ...
        audience,
        language,
        existingWording: ctx.existing,
        keyMessage: keyMessage || undefined,
        businessDescription: ctx.businessDescription,
        preferences: ctx.preferences,
        alternativesCount: 3,
      };
      const { result, generationId } = await generate({
        data: { businessId: ctx.businessId, packId: ctx.packId, input },
      });
      setResponse(result);
  ...
  async function apply(a: Alternative, index: number, patch: ApplyPatch) {
    ...
    if (generationId) {
      try {
        await mark({ data: { generationId, alternativeIndex: index } });
      } catch {
        /* ignore */
      }
    }
  ...
  async function saveFavourite(a: Alternative) {
    ...
    const { error } = await supabase.from("ai_copy_favourites").insert({
      ...
      name,
      headline: a.headline,
      support_text: a.supportingText,
      cta_text: a.ctaText,
      footer_text: a.footerText,
      tone: a.tone || tone,
      placement,
    } as never);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              maxLength={80}
              className="rounded-xl"
              placeholder="e.g. weekend brunch crowd"
            />
          </div>
          <div className="sm:col-span-2 space-y-1">
            ...
            <Textarea
              ...
              maxLength={280}
              className="rounded-xl"
              placeholder="What should the wording emphasise?"
            />
          </div>
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-2xl bg-accent/30 p-3 text-[11px] text-muted-foreground">
          <span>
            Recommended max — h {limits.headline} · sup {limits.supportingText} · cta{" "}
            {limits.ctaText} · foot {limits.footerText}
          </span>
        ...
        {response && (
          <div className="mt-3 space-y-3">
            {(response.safety.reviewGatingDetected ||
              response.safety.incentiveDetected ||
              response.safety.fakeReviewDetected) && (
              <div className="rounded-2xl border border-amber-500/40 bg-amber-500/10 p-3 text-xs text-amber-500">
                <AlertTriangle className="mr-1 inline h-3 w-3" />
                Some alternatives were flagged as unsafe (review gating / incentives / fake
                wording). Unsafe cards are marked and cannot be applied.
              </div>
            )}
            {response.alternatives.map((a, i) => (
              <AlternativeCard
                ...
                a={a}
                placement={placement}
                onApply={(patch) => apply(a, i, patch)}
                onFavourite={() => saveFavourite(a)}
              />
            ))}
          </div>
        )}
...
function AlternativeCard({
    ...
    <div className="flex items-start justify-between gap-2 rounded-xl bg-accent/20 px-3 py-2">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-wide text-muted-foreground">
          {label}
          <span className={over(val, max) ? "text-amber-500" : "text-muted-foreground"}>
            {val.length}/{max}
          </span>
        </div>
      ...
      <Button
        ...
        variant="ghost"
        disabled={!safe || !val}
        onClick={() => onApply({ [k]: val } as ApplyPatch)}
  ...
  return (
    <div
      className={`rounded-2xl border p-3 ${safe ? "border-border/60" : "border-destructive/40 bg-destructive/5"}`}
    >
      ...
      <div className="grid gap-1.5">
        ...
        <Field
          k="supportingText"
          label="Supporting"
          max={limits.supportingText}
          val={a.supportingText}
        />
      ...
      {a.rationale && (
        <p className="mt-2 text-[10px] italic text-muted-foreground">{a.rationale}</p>
      )}
    </div>
  );
}
...
function errorMessage(code: string): string {
  switch (code) {
    case "monthly_limit_reached":
      return "You've hit the monthly generation limit. Try again next month or contact support.";
    case "rate_limit_reached":
      return "Too many generations in the last hour. Please wait and try again.";
    case "rate_limit_upstream":
      return "The AI service is busy. Please retry in a moment.";
    case "credits_exhausted":
      return "AI credits exhausted. Please contact your workspace admin.";
    case "invalid_response_format":
      return "The AI returned an unexpected format. Please regenerate.";
    default:
...
export function AiCopyHistoryPanel({
  businessId,
  onReapply,
}: {
  businessId: string | null;
  onReapply: (a: Alternative) => void;
}) {
  ...
  return (
    <div className="space-y-2">
      ...
      {rows.length === 0 && (
        <p className="rounded-2xl border border-dashed p-4 text-center text-[11px] text-muted-foreground">
          No generations yet.
        </p>
      )}
      {rows.map((r) => (
        <div key={r.id} className="rounded-2xl border border-border/60 p-2 text-xs">
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-muted-foreground">
              {new Date(r.created_at).toLocaleString()} · {r.tone} · {r.placement}
            </span>
            <Button
              size="sm"
              variant="ghost"
          ...
          <div className="mt-1 space-y-1">
            {(r.generated_output?.alternatives ?? []).slice(0, 3).map((a, i) => (
              <button
                key={i}
                onClick={() => onReapply(a)}
                className="block w-full rounded-xl bg-accent/30 p-2 text-left hover:bg-accent"
              >
