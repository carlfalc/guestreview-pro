...
export function FoldedFormatEditor(props: Props) {
  ...
  const [draft, setDraft] = useState<FoldedConfig>(
    () => config ?? defaultFoldedConfig(contentBase),
  );
  const [tab, setTab] = useState("front");
  useEffect(() => {
    if (open) setDraft(config ?? defaultFoldedConfig(contentBase));
  }, [open, config, contentBase]);
  ...
  function setMode(mode: FoldedMode) {
    setDraft((d) => ({ ...d, mode }));
  }
      ...
      <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
          ...
          <Tabs value={tab} onValueChange={setTab}>
            <TabsList className="w-full">
              ...
              <TabsTrigger
                value="back"
                className="flex-1"
                disabled={backLocked && draft.mode === "same_both_sides"}
              >
                <PanelBottom className="mr-1.5 h-3.5 w-3.5" />
                Back
              </TabsTrigger>
        ...
        {!layout && (
          <p className="text-[10px] text-destructive">
            No folded layout registered for this format.
          </p>
        )}
      </DialogContent>
    </Dialog>
...
function ModeSelector({
  ...
  value: FoldedMode;
  onChange: (v: FoldedMode) => void;
}) {
  const options: { id: FoldedMode; label: string; desc: string }[] = [
    {
      id: "same_both_sides",
      label: "Same both sides",
      desc: "Front and back share the same content",
    },
    {
      id: "mirrored",
      label: "Mirrored layout",
      desc: "Same content, back rotated 180° for readability",
    },
    {
      id: "different_sides",
      label: "Different front and back",
      desc: "Edit front and back independently",
    },
  ];
  return (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-2 rounded-xl bg-accent/30 p-3 sm:grid-cols-2">
        ...
        <ChkRow
          label="Show logo"
          value={p.showLogo !== false}
          onChange={(v) => onChange("showLogo", v)}
          disabled={disabled}
        />
        <ChkRow
          label="Show business name"
          value={p.showBusinessName !== false}
          onChange={(v) => onChange("showBusinessName", v)}
          disabled={disabled}
        />
        <ChkRow
          label="Show stars"
        ...
        <ChkRow
          ...
          value={p.showGoogleBadge !== false}
          onChange={(v) => onChange("showGoogleBadge", v)}
          disabled={disabled}
        />
      </div>
      ...
      <div className="grid gap-3 rounded-xl border border-border/60 p-3 sm:grid-cols-3">
        <SliderRow
          label="QR size"
          value={p.qrScale ?? 0.45}
          min={0.3}
          max={0.7}
          step={0.01}
          onChange={(v) => onChange("qrScale", v)}
          disabled={disabled}
        />
        <SliderRow
          label="QR horizontal"
          ...
          max={0.35}
          step={0.01}
          onChange={(v) => onChange("qrOffsetX", v)}
          disabled={disabled}
        />
        <SliderRow
          label="QR vertical"
          ...
          max={0.35}
          step={0.01}
          onChange={(v) => onChange("qrOffsetY", v)}
          disabled={disabled}
        />
      </div>
      ...
      {p.backgroundImage && (
        <SliderRow
          ...
          value={p.backgroundImageOpacity ?? 1}
          min={0}
          max={1}
          step={0.05}
          onChange={(v) => onChange("backgroundImageOpacity", v)}
          disabled={disabled}
        />
      )}
    </div>
    ...
    <div className="grid gap-4 md:grid-cols-[220px_1fr]">
      <div className="space-y-2 text-[11px]">
        ...
        <dl className="space-y-1">
          ...
          <Row
            k="Score lines"
            v={String(layout.segments.filter((s) => s.type === "score").length)}
          />
      ...
      <div
        className="rounded-xl border border-border/70 bg-white p-2 [&_svg]:h-auto [&_svg]:w-full"
...
function renderStructureSvg(
  ...
  g: { fold: boolean; score: boolean; cut: boolean; glue: boolean; labels: boolean; safe: boolean },
): string {
  ...
  parts.push(
    `<rect x="${b}" y="${b}" width="${layout.flatWidth}" height="${layout.flatHeight}" fill="#ffffff" stroke="#cbd5e1" stroke-width="0.3"/>`,
  );
  if (g.safe) {
    for (const p of layout.panels) {
      const s = layout.safeInset;
      parts.push(
        `<rect x="${b + p.x + s}" y="${b + p.y + s}" width="${p.w - s * 2}" height="${p.h - s * 2}" fill="none" stroke="#22c55e" stroke-width="0.3" stroke-dasharray="1.5 1.5"/>`,
      );
    }
  }
  for (const s of layout.segments) {
    ...
    parts.push(
      `<line x1="${b + s.x1}" y1="${b + s.y1}" x2="${b + s.x2}" y2="${b + s.y2}" stroke="${color}" stroke-width="${s.type === "cut" ? 0.4 : 0.5}"${dash ? ` stroke-dasharray="${dash}"` : ""}/>`,
    );
  }
  if (g.glue && layout.glue) {
    parts.push(
      `<rect x="${b + layout.glue.x}" y="${b + layout.glue.y}" width="${layout.glue.w}" height="${layout.glue.h}" fill="#3b82f6" fill-opacity="0.15" stroke="#3b82f6" stroke-dasharray="2 2" stroke-width="0.3"/>`,
    );
  }
  if (g.labels) {
    for (const p of layout.panels) {
      parts.push(
        `<text x="${b + p.x + 3}" y="${b + p.y + 6}" font-family="Inter, sans-serif" font-size="4" font-weight="700" fill="#64748b">${p.label.toUpperCase()}</text>`,
      );
    }
  }
  parts.push(`</svg>`);
...
function PreviewView(props: {
  ...
  useEffect(() => {
    ...
    const input = {
      ...
      qrDesign: props.qrDesign,
      qrData: props.qrData,
      qrLogoUrl: props.qrLogoUrl,
      config: props.config,
    };
    const p =
      mode === "mockup"
        ? renderFoldedMockupSvg(input)
        : renderFoldedFormatSvg(input, {
            facing: mode,
            includeBleed: mode === "flat",
            showFold: mode === "flat",
            showCut: mode === "flat",
            showSafe: mode === "flat",
            showPanelLabels: mode === "flat",
          });
    p.then((s) => {
      if (!cancelled) {
        setSvg(s);
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-1.5">
        {opts.map((o) => (
          <button
            ...
            className={`rounded-full border px-3 py-1 text-[11px] ${mode === o.id ? "border-primary bg-primary text-primary-foreground" : "border-border hover:bg-accent"}`}
          >
            {o.label}
          </button>
        ))}
      ...
      {mode === "mockup" && (
        <p className="text-[10px] text-muted-foreground">
          Shows front and back assembled faces side by side.
        </p>
      )}
...
function ChkRow({
  ...
  return (
    <label className="flex items-center gap-2 text-xs">
      <Checkbox checked={value} onCheckedChange={(v) => onChange(v === true)} disabled={disabled} />{" "}
      {label}
    </label>
  );
}
