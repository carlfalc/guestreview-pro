// ISO 3166-1 alpha-2 country names. Used so that unsupported countries still
// display a proper country name (e.g. "Argentina") instead of a bare code.
...
export function countryName(code: string | null | undefined): string {
  ...
  return ISO_COUNTRY_NAMES[up] ?? up;
}

export function isValidCountryCode(code: unknown): code is string {
  if (typeof code !== "string") return false;
  const up = code.toUpperCase();
  return /^[A-Z]{2}$/.test(up) && (up in ISO_COUNTRY_NAMES || up === "ZZ");
}
