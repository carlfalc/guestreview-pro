...
export function placementFromFormat(f: BusinessFormat | null | undefined): Placement {
  ...
  if (f.category === "hotel")
    return f.id.includes("reception") ? "Reception sign" : "Hotel room card";
...
export function detectSafetyIssues(input: {
  headline?: string;
  supportingText?: string;
  ctaText?: string;
  footerText?: string;
}): SafetyFlags {
  const text = [input.headline, input.supportingText, input.ctaText, input.footerText]
    .filter(Boolean)
    .join(" \n ");
