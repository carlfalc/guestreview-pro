...
import { getFoldedLayout, type PanelRect, type FoldSegment } from "@/lib/folded-layouts";
...
function targetFor(key: string): ValidationTarget {
  ...
  if (key === "headline" || key === "supportText" || key === "footer" || key === "cta")
    return "override";
  return "override";
}
...
function validatePanel(
  ...
  const glueLocal =
    ...
    rectsOverlap(
      ...
      ? ({
          key: "glue",
          label: "Glue area",
          x: layout.glue.x - panel.x,
          y: layout.glue.y - panel.y,
          w: layout.glue.w,
          h: layout.glue.h,
        } as PanelBox)
      : null;
  ...
  for (const b of boxes) {
    ...
    const outsideSafe =
      b.x < safeInset ||
      b.y < safeInset ||
      b.x + b.w > panel.w - safeInset ||
      b.y + b.h > panel.h - safeInset;
    if (outsideSafe) {
      out.push(
        mk(
          "warning",
    ...
    for (const f of foldLocal) {
      ...
      if (
        segmentIntersectsRect(f, b) &&
        b.x > 0.01 &&
        b.y > 0.01 &&
        b.x + b.w < panel.w - 0.01 &&
        b.y + b.h < panel.h - 0.01
      ) {
        out.push(
          mk(
            "error",
            ...
            b.key,
            b.label,
            `${b.label} crosses a fold line`,
            "will be creased when folded.",
            "Move the element off the fold line.",
          ),
        );
      }
    }
      ...
      if (segmentIntersectsRect(s, b)) {
        out.push(
          mk(
            ...
            b.key,
            b.label,
            `${b.label} crosses a score line`,
            "will look pinched at the score.",
            "Move the element off the score line.",
          ),
        );
      }
    }
    if (glueLocal && rectsOverlap(b, glueLocal)) {
      out.push(
        mk(
          ...
          b.key,
          b.label,
          `${b.label} enters the glue area`,
          "sits on the adhesive area and will not be visible when assembled.",
          "Move the element out of the glue area.",
        ),
      );
    }
  }
      ...
      if (rectsOverlap(b, qr)) {
        out.push(
          mk(
            ...
            b.key,
            b.label,
            `${b.label} overlaps the QR`,
            "overlaps the QR quiet zone and will break scanning.",
            `Move ${b.label.toLowerCase()} away from the QR or reduce QR scale.`,
          ),
        );
      }
    }
      ...
      if (rectsOverlap(a, b)) {
        out.push(
          mk(
            ...
            `overlaps with ${b.label.toLowerCase()} — text will collide.`,
            "Shorten one of the strings or adjust layout.",
          ),
        );
      }
    }
  ...
  if (panel.rotation !== 0) {
    ...
    out.push({
      id: `${panelName.toLowerCase()}-orientation`,
      formatId: format.id,
      formatName: `${format.name} · ${panelName}`,
      category: "print",
      level: "warning",
...
export function runFoldedValidations(input: FoldedValidationInput): ValidationResult[] {
  ...
  if (!front || !back) {
    out.push({
      ...
      category: "print",
      level: "error",
      title: "Panel geometry invalid",
      message: "Front or back panel is missing from the folded layout.",
      suggestedFix: "Contact support — this format has an invalid layout definition.",
    });
    return out;
  }
  if (Math.abs(front.w - back.w) > 0.1 || Math.abs(front.h - back.h) > 0.1) {
    out.push({
      id: "structure-panel-size-mismatch",
      formatId: format.id,
      formatName: format.name,
      category: "print",
      level: "error",
      title: "Front and back panel sizes differ",
      message: "Folded tents require front and back panels of equal size for a clean fold.",
      suggestedFix: "Contact support — layout definition is inconsistent.",
    });
  }
  if (!layout.segments.some((s) => s.type === "cut")) {
    out.push({
      ...
      category: "print",
      level: "error",
      title: "Missing cut path",
      message: "Layout does not define a CutContour.",
      suggestedFix: "Contact support.",
    });
  }
  if (!layout.segments.some((s) => s.type === "fold")) {
    out.push({
      id: "structure-no-fold",
      formatId: format.id,
      formatName: format.name,
      category: "print",
      level: "error",
      title: "Missing fold path",
      message: "Layout does not define a FoldLine.",
      suggestedFix: "Contact support.",
    });
  }
  if (format.bleed <= 0) {
    out.push({
      id: "structure-no-bleed",
      formatId: format.id,
      formatName: format.name,
      category: "print",
      level: "warning",
      title: "Bleed is 0",
      message: "Folded pieces normally need 3 mm bleed for commercial print.",
      suggestedFix: "Set bleed to 3 mm.",
    });
  }
  ...
  if (!qrData) {
    out.push({
      ...
      message: "This QR has no short-link and cannot resolve when scanned.",
      suggestedFix: "Save the QR code to generate a short-link.",
      element: "qr",
      target: "qr",
    });
  }
  ...
  if (ratio < 3) {
    out.push({
      ...
      message: `Contrast ratio ${ratio.toFixed(2)}:1 — scanners fail below 3:1.`,
      suggestedFix: "Switch QR to black on white.",
      element: "qr",
      target: "qr",
    });
  } else if (ratio < 4.5) {
    out.push({
      ...
      category: "qr",
      level: "warning",
      title: "QR contrast is borderline",
      message: `Contrast ratio ${ratio.toFixed(2)}:1.`,
      suggestedFix: "Aim for 4.5:1 or higher.",
      element: "qr",
      target: "qr",
    });
  }
...
async function decodeFace(
  ...
  try {
    ...
    return {
      pass: false,
      reason: `Rendered ${facing} face did not decode. Increase QR scale, restore quiet zone, or reduce logo size.`,
    };
  } catch (e) {
 ...
 */
export async function decodeFoldedQrValidation(
  input: FoldedRenderInput,
): Promise<FoldedDecodeResult> {
  ...
  results.push(
    front.pass
      ? {
          id: `${format.id}-front-decode`,
          formatId: format.id,
          formatName: `${format.name} · Front`,
          category: "qr",
          level: "pass",
          title: "Front: QR decodes",
          message: "Assembled front face decoded successfully.",
          element: "qr",
        }
      : {
          id: `${format.id}-front-decode`,
          ...
          category: "qr",
          level: "error",
          title: "Front: QR could not be decoded",
          message: front.reason ?? "Front face failed to scan.",
          suggestedFix: "Increase QR scale or reduce logo size.",
          element: "qr",
          target: "override",
        },
  );
  results.push(
    back.pass
      ? {
          id: `${format.id}-back-decode`,
          formatId: format.id,
          formatName: `${format.name} · Back`,
          category: "qr",
          level: "pass",
          title: "Back: QR decodes (upright)",
          message: "Assembled back face decoded upright.",
          element: "qr",
        }
      : {
          id: `${format.id}-back-decode`,
          ...
          category: "qr",
          level: "error",
          title: "Back: QR could not be decoded",
          message: back.reason ?? "Back face failed to scan.",
          suggestedFix: "Check back-panel orientation and QR scale.",
          element: "qr",
          target: "override",
        },
  );
