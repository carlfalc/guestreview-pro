...
import {
  ...
  AlertTriangle, CheckCircle2, Copy, Archive,
} from "lucide-react";
...
import {
  PACK_TYPES, statusMeta, packTypeById,
  type GlobalSettings, type FormatCustomizations, type PackStatus, type PackType,
...
function MarketingPackEditor() {
  ...
  const logoUrl = showLogo ? (qrRow?.logo_url ?? biz?.logo_url ?? null) : null;
  const qrData = typeof window !== "undefined" && qrRow ? `${window.location.origin}/r/${qrRow.short_code}` : "";

  const content: FormatContent = useMemo(() => ({
    businessName: showBusinessName ? (biz?.name ?? "") : "",
    logoUrl,
    headline,
    supportText,
    ctaText,
  }), [showBusinessName, biz, logoUrl, headline, supportText, ctaText]);
  ...
  const doSave = useCallback(async (): Promise<boolean> => {
    ...
    return true;
  }, [pack, projectName, layoutTemplate, headline, supportText, ctaText, footerText, showBusinessName, showLogo, showStars, showGoogleBadge, selectedFormats, globalSettings, formatCustomizations, status, qc, id]);
  ...
  async function validateAllQrs(): Promise<{ formatId: string; pass: boolean; reason?: string }[]> {
    ...
    for (const f of selected) {
      try {
        const svg = await renderFormatSvg(f, layoutTemplate, content, qrDesign, qrData, logoUrl, brand, { showBoundaries: false, includeBleed: false });
        const target = 600;
  ...
  async function exportZip(kind: "all" | "print" | "digital") {
    ...
    try {
      await downloadPackZip(projectName || "marketing-pack", list, layoutTemplate, content, qrDesign, qrData, logoUrl, brand);
...
function FormatPreviewCard(props: {
  ...
  async function validate() {
    ...
    try {
      ...
      setScanStatus("fail"); setScanReason((e as Error).message);
    }
  }
  ...
  return (
    <div className="rounded-2xl border border-border/70 bg-card p-3">
      ...
      <div className="mt-2 grid grid-cols-4 gap-1.5">
        <Button size="sm" variant="outline" onClick={() => run("png")} disabled={exporting !== null} className="rounded-full text-[10px]">
          {exporting === `${format.id}-png` ? <Loader2 className="h-3 w-3 animate-spin"/> : <ImageIcon className="h-3 w-3"/>}
        </Button>
        <Button size="sm" variant="outline" onClick={() => run("svg")} disabled={exporting !== null} className="rounded-full text-[10px]">
          {exporting === `${format.id}-svg` ? <Loader2 className="h-3 w-3 animate-spin"/> : <Download className="h-3 w-3"/>}
        </Button>
        <Button size="sm" variant="outline" onClick={() => run("pdf")} disabled={exporting !== null} className="rounded-full text-[10px]">
