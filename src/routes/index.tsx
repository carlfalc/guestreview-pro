...
import { PublicShell, Section, CardGrid, InfoCard, Faq, FinalCta } from "@/components/public/PublicShell";
...
export const Route = createFileRoute("/")({
  ...
  head: () =>
    seo({
      ...
      jsonLd: [jsonLd.organization(), jsonLd.website(), jsonLd.softwareApplication(), jsonLd.faq(FAQS)],
    }),
});
  ...
  return (
    <div className="relative mx-auto flex items-center justify-center" style={{ width: 300, height: 300 }}>
      ...
      <span
        ...
        style={{ background: "conic-gradient(from 90deg, #4285F4, #34A853, #FBBC05, #EA4335, #4285F4)" }}
      />
...
function Landing() {
  return (
    <PublicShell>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-6 py-16 sm:py-24 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-5 max-w-xl text-lg leading-relaxed text-white/65">
              Design a branded QR code that sends customers directly to your Google review form, print it
              on stickers, table tents and counter cards, and see exactly where scans come from.
            </p>
      ...
      >
        <CardGrid>
          <InfoCard title="Stickers and decals" body="Circular and square stickers plus window decals, with bleed, safe area and cut-line dielines included in every export." />
          <InfoCard title="Folded table tents" body="True folded geometry for A5 and A6 tents — front and back panels, fold lines and correct production rotation." />
          <InfoCard title="Counter and reception cards" body="Rigid card layouts for checkouts, front desks and bars, sized for standard print stock." />
          <InfoCard title="Posters and lift signage" body="Larger-format layouts with QR sizing checked against expected scan distance." />
          <InfoCard title="Hotel room cards" body="Bedside and compendium cards designed for the moment a guest is most likely to leave feedback." />
          <InfoCard title="Digital assets" body="Web and email-signature versions of the same code, so online and printed touchpoints stay consistent." />
        </CardGrid>
      </Section>

      {/* Analytics */}
      <Section title="Analytics preview" intro="Know which placements actually earn scans before you reprint anything.">
        ...
        <p className="mt-4 text-xs text-white/40">
          Illustrative sample data shown for demonstration. Your dashboard shows only your own scans.
        </p>
      </Section>

      {/* Industry use cases */}
      <Section title="Built for the places people leave reviews" intro="Placement guidance and templates tuned to how each venue actually operates.">
        <CardGrid cols={4}>
          {[
            { t: "Hotels & motels", b: "Reception, room compendium, bedside card, lift poster, checkout email." },
            { t: "Restaurants & cafés", b: "Table tents, counter cards, bill folders, window decals, takeaway packaging." },
            { t: "Bars & tourism", b: "Bar-top stickers, menu inserts, tour desks and ticket counters." },
            { t: "Retail & services", b: "Checkout counters, receipts, packaging inserts, window stickers, email signatures." },
          ].map((item) => (
      ...
      <Section title="Simple pricing" intro="Start free. Upgrade when one code stops being enough.">
        <CardGrid>
          {[
            { name: "Free", price: "US$0", tagline: "1 business, 1 QR code", features: ["Basic analytics", "Basic marketing packs", "No card required"] },
            { name: "Pro", price: "from US$19/mo", tagline: "For a growing venue", features: ["Unlimited QR codes", "Advanced analytics", "AI copy assistant", "No GuestReview Pro branding"] },
            { name: "Business", price: "from US$49/mo", tagline: "For multi-location operators", features: ["Up to 10 businesses", "Portfolio reporting", "White-label exports", "Team accounts"] },
          ].map((plan) => (
      ...
      <Section title="Why teams choose GuestReview Pro">
        <CardGrid>
          <InfoCard icon={<Sparkles className="h-5 w-5" aria-hidden />} title="AI copy assistant" body="Review-request wording in several tones, written to fit the exact format you are printing." />
          <InfoCard icon={<Wand2 className="h-5 w-5" aria-hidden />} title="Print validation" body="Contrast, quiet zone, bleed and decode tests run before you export — not after the print run." />
          <InfoCard icon={<LineChart className="h-5 w-5" aria-hidden />} title="Per-placement analytics" body="One code per table, room or counter, so you can compare placements honestly." />
        </CardGrid>
      </Section>
    ...
    <div className="mx-auto mt-6 max-w-md rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-[0_30px_80px_-40px_rgba(0,0,0,0.9)]">
      ...
      <div className="mt-4 flex items-center gap-4">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-2 text-xs text-[#34A853]">Ready to print · contrast and quiet zone passed</p>
        </div>
      </div>
    </div>
  );
...
function SampleQr() {
  const cells = [
    "1111111011010001111111", "1000001010111010000001", "1011101001001010111011",
    "1011101011100010111011", "1011101000110010111011", "1000001011011010000001",
    "1111111010101011111111", "0000000001110000000000", "1101101110011011011011",
    "0110010001100100110010", "1011011011010110110110", "0100100110101101001001",
    "1101101011011011011011", "0000000010110100000000", "1111111001101011011011",
    "1000001011010010010010", "1011101001101101101101", "1011101010010110010110",
    "1011101011101001101001", "1000001000110110110110", "1111111011011011011011",
  ];
  return (
    <svg viewBox="0 0 22 21" className="h-[78%] w-[78%]" role="img" aria-label="Example QR code design">
      <rect width="22" height="21" fill="#ffffff" />
      {cells.map((row, y) =>
        row.split("").map((cell, x) =>
          cell === "1" ? <rect key={`${x}-${y}`} x={x} y={y} width="1" height="1" fill="#0a0f3d" /> : null,
        ),
      )}
    </svg>
  );
